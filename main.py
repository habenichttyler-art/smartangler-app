...
# --- HARDCODED STATEWIDE REGIONAL TELEMETRY ROUTING MAP ---
...
def get_isolated_county_nodes(county):
    ...
    # Load completely static, zero-offset hardware entries directly from JSON file matrix
    with open("coordinates.json", "r") as f:
        master_water_matrix = json.load(f)
    ...
    for node in raw_node_sheet:
        ...
        compiled_nodes.append({
            ...
            "structures": [{"path": [[lat - 0.0003, lon - 0.0003], [lat, lon]], "name": "Submerged Channel Drop Edge"}],
            "highways": [{"path": [[lat - 0.0006, lon + 0.0006], [lat, lon]], "name": "Bait Migration Current Runway"}],
            "labels": f"Geospatial Node Verified On Water // Station {tide_id}"
        })
